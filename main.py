...
class DiaryWindow(Screen):
    def on_enter(self):
        logging.debug("Entered DiaryWindow")
        self.diary_display()
        self.update_date(MDApp.get_running_app())

    def update_date(self, app):
        self.ids.date_button.text = app.date_text

    def diary_display(self):
        logging.debug("Displaying diary")
        recorded_foods = fc.select_food()
        logging.debug(f"Recorded foods: {recorded_foods}")
        container = self.ids.container
        container.clear_widgets()
        logging.debug("Cleared previous widgets")

        if len(recorded_foods):
            for recorded_food in recorded_foods:
                row: FoodModel = FoodModel(*recorded_food)
                food_item = ThreeLineIconListItem(
                    IconLeftWidget(icon="food"
                                   ),
                    text=f"{row.meal_category}: {row.food_name}",
                    secondary_text=f"Calorie: {row.calories}",
                    tertiary_text=f"Date Taken: {row.created_at}"
                )
                container.add_widget(food_item)
            logging.debug("Recorded foods processed and displayed")
        else:
            logging.debug("No recorded foods found")
            container.add_widget(OneLineListItem(text="No food items recorded."))
...
sm.add_widget(DiaryWindow(name="diary"))
...
class FitnessApp(MDApp):
    ...
    def schedule_clearing(self):
        Clock.schedule_interval(self.clear_food_containers, 86400)  # 86400 seconds = 24 hours

    def clear_food_containers(self, dt):
        try:
            diary_screen = self.root.get_screen("diary")
            container = diary_screen.ids.container
            container.clear_widgets()
        except Exception as e:
            logging.error(f"Error clearing food containers: {e}")
        ...
        if food_name and food_quantity and meal_category and calories_label:
            try:
                ...
                if res:
                    ...
                    diary_screen = self.root.get_screen('diary')
                    diary_screen.diary_display()

                    self.root.current = "calorie"

                    logging.debug("Food item saved and diary updated.")
            except Exception as e:
                logging.error(f"Error saving food: {e}")
